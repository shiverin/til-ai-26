...
def test_try_compile_encoder_swaps_encoder_on_success(monkeypatch):
    """Happy path: torch.compile returns a wrapper; we swap encoder for it."""
    manager = ASRManager.__new__(ASRManager)

    sentinel_original = object()
    sentinel_compiled = object()
    manager.model = type("M", (), {"encoder": sentinel_original})()

    captured = {}
    def fake_compile(mod, mode=None, dynamic=None, **kwargs):
        captured["mod"] = mod
        captured["mode"] = mode
    ...
    assert manager._try_compile_encoder() is True
    assert manager.model.encoder is sentinel_compiled
    assert captured["mod"] is sentinel_original
    assert captured["mode"] == "reduce-overhead"
    assert captured["dynamic"] is True


def test_try_compile_encoder_swallows_failure_and_keeps_eager(monkeypatch):
    """Fallback: torch.compile raises; encoder attribute unchanged, returns False."""
    manager = ASRManager.__new__(ASRManager)
    ...
    def fake_compile(*a, **kw):
        raise RuntimeError("inductor not available")

    monkeypatch.setattr(torch, "compile", fake_compile)

    assert manager._try_compile_encoder() is False
    assert manager.model.encoder is sentinel_original
