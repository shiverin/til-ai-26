...
class ASRManager:
    ...
    def __init__(self):
        ...
        # Hardcoded batch size + single warmup transcribe. The earlier
        # multi-candidate probe was eating ~30-60s of startup time (each
        # failed candidate OOMs, recovers, retries the next), which the
        # leaderboard speed score appears to charge — submissions using
        # the simple original asr_manager scored 0.943, and every probe
        # variant since landed at 0.930-0.934. Batch 16 is what we shipped
        # successfully for weeks; the probe added no observed value.
        self._batch_size = 16
        try:
            warmup_bytes = [self._silence_wav(8.0)] * self._batch_size
            warmup_signals = [self._decode(b) for b in warmup_bytes]
            with torch.autocast(
            ...
            _LOG.warning("warmup skipped: %s", exc)
