...
class HybridPPOConfig:
    ...
    clip_coef: float = 0.2
    ent_coef: float = 0.02               # current-update value (caller anneals 0.05->0.005)
    ent_coef_final: float = 0.005
    vf_coef: float = 0.5
    max_grad_norm: float = 0.5
    target_kl: float = 0.02              # minibatch-level running-KL abort threshold
    kl_lr_halve_factor: float = 2.0      # halve actor LR if running KL > factor*target_kl
...
def ppo_update_hybrid(actor, critic, opt, buf, advantages, returns, cfg, device,
    ...
    for _ in range(cfg.update_epochs):
        ...
        for start in range(0, n, mb_size):
            ...
            # applied, not rolled back (standard PPO early-stop); the LR-halve
            # takes effect on subsequent updates.
            if cfg.target_kl is not None and running_kl > cfg.target_kl:
                if running_kl > cfg.kl_lr_halve_factor * cfg.target_kl:
