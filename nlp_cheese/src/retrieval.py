...
# Embedding batch size. Larger batches keep the GPU saturated — the corpus
# index embeds thousands of chunks, so this directly cuts corpus-load time.
EMBED_BATCH = 256
...
class Retriever:
    ...
    def __init__(self, embedder, reranker=None, dense_top=32, sparse_top=32,
                 fuse_top=24, final_k=6):
        # reranker=None skips the cross-encoder stage: candidates are taken
        # straight from the RRF fusion. Faster, smaller, slightly lower recall.
    ...
    def retrieve(self, question):
        ...
        if self.reranker is None:
            self.last_top_score = None
            return [self.chunks[i] for i in fused[:self.final_k]]
    ...
    def retrieve_batch(self, questions):
        ...
        # No reranker: take the RRF-fused top candidates directly.
        if self.reranker is None:
            for qi, fused in enumerate(fused_per_q):
                results[qi] = [self.chunks[ci] for ci in fused[:self.final_k]]
            return results
        ...
        for qi in range(len(questions)):
            ranked = sorted(scored[qi], key=lambda x: x[1],
                            reverse=True)[:self.final_k]
            results[qi] = [self.chunks[ci] for ci, _ in ranked]
        return results
