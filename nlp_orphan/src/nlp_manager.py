...
# Overridable via NLP_GPU_MEM_FRACTION so a run can shrink its footprint when
# the T4 is shared with another job (a recurring situation here).
GPU_MEM_FRACTION = float(os.environ.get("NLP_GPU_MEM_FRACTION", "0.70"))
# Measured worst case over all 883 questions: prompt 1419 tok + 256 output =
# 1675. 2048 leaves a 373-token margin. A smaller max_model_len means vLLM
# reserves less KV per sequence, so more sequences run concurrently
# (~6.5x -> ~9.7x at the current KV-cache size) — free, no accuracy effect.
MAX_MODEL_LEN = int(os.environ.get("NLP_MAX_MODEL_LEN", "2048"))
# Max concurrent sequences vLLM schedules. This bounds the sampler-warmup
# allocation and is the main batched-throughput knob; overridable for tuning.
MAX_NUM_SEQS = int(os.environ.get("NLP_MAX_NUM_SEQS", "64"))
# enforce_eager=True skips CUDA-graph capture (saves ~1.5 GB VRAM). Setting it
# False captures graphs for a faster decode loop, at a VRAM cost — worth it
# when the T4 is not shared. Overridable so it can be A/B-benchmarked.
ENFORCE_EAGER = os.environ.get("NLP_ENFORCE_EAGER", "1") == "1"
# Optional RAFT LoRA adapter. Empty = base model (default; baseline behavior).
# Set NLP_LORA_PATH to an adapter directory to serve the fine-tuned generator.
LORA_PATH = os.environ.get("NLP_LORA_PATH", "")
LORA_RANK = 16  # must be >= the adapter's r (raft_train.py uses r=16)
...
class NLPManager:
    ...
    def __init__(self):
        ...
        #
        # NOTE: ngram speculative decoding was trialled (2026-05-19) and gave no
        # measurable speedup at batch>=4 — and it disables vLLM's async
        # scheduler, which would hurt throughput at the larger batch sizes we
        # target. Not used. See PROGRESS.md.
        self.llm = LLM(
        ...
        # LoRARequest passed on every generate call when an adapter is set.
        self.lora_req = (LoRARequest("raft", 1, LORA_PATH)
                         if LORA_PATH else None)
        if LORA_PATH:
            print(f">>> NLPManager: serving LoRA adapter {LORA_PATH}",
                  flush=True)
    ...
    def load_corpus(self, documents):
        ...
        # Per-doc summary (first 600 chars: title + classification + metadata
        # blocks like `**Subject:**`/`**RE:**`) feeds a doc-level BM25 used to
        # rescore retrieval — fixes cases where the canonical doc loses to
        # docs that mention the topic in passing.
        doc_summaries = [t[:600] for t in texts]
