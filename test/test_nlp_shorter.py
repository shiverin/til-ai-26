...
BATCH_SIZE = 4
N_TESTCASES = 10  # shorter smoke test: only run the first N questions
...
def main():
    ...
    # shorter smoke test: keep only the first N_TESTCASES questions
    instances = instances[:N_TESTCASES]
    logger.info(f"SHORTER TEST: running only {len(instances)} test cases")
    ...
    results_path = results_dir / "nlp_results_shorter.json"
